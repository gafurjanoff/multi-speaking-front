...
export function ResultRecordingsView({ parts, recordings = [], showScores = false, aiScores = {} }: ResultRecordingsViewProps) {
  const recByPartQuestion = new Map<string, { file_path: string | null; duration: number; score?: number | null; feedback?: string | null }>()
  ...
  for (const r of recordings) {
    ...
    recByPartQuestion.set(key, { file_path: r.file_path, duration: r.duration, score: ai?.score ?? r.score, feedback: feedbackDisplay(ai?.feedback ?? r.feedback) ?? ai?.feedback ?? r.feedback ?? undefined })
      ...
      <div className="space-y-4">
        {recordings.map((rec, idx) => (
          <div key={rec.id} className="rounded-xl border border-border bg-muted/20 p-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{rec.part_label || `Part ${idx + 1}`}</p>
        ...
        <div key={part.part_id} className="rounded-2xl border border-border bg-muted/5 overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {part.questions.map((q, qIdx) => {
              ...
              const images = isPart1Photos ? [] : ((part.part_images && part.part_images.length > 0 ? part.part_images : q.images) ?? [])
              ...
              let fallbackRec = recByPartQuestion.get(`${String(part.part_id).trim().toLowerCase()}:${String(q.question_id).trim().toLowerCase()}`)
              if (!fallbackRec && partIdx < recsByPartOrder.length) {
                ...
                if (partRecs && qIdx < partRecs.length) {
                  ...
                  const ai = aiScores[r.id]
                  fallbackRec = { file_path: r.file_path, duration: r.duration, score: ai?.score ?? r.score, feedback: ai?.feedback ?? r.feedback }
                }
              }
              ...
              const fallbackFeedback = feedbackDisplay(fallbackRec?.feedback ?? q.feedback) ?? fallbackRec?.feedback ?? q.feedback
              return (
